# Cargo-culted from: ampt, boztek
# See also: frostbytten
...
class Php <Formula
  ...
  def options
   [
     ...
     ['--with-mssql', 'Build with SQL Server support'],
  ...
  def configure_args
    ...
    if ARGV.include? '--with-mssql'
      args.push "--with-mssql"
      args.push "--with-pdo-dblib"
      args.push "--with-unixODBC"
      args.push "--with-pdo-odbc=unixODBC"
    end

    if ARGV.include? '--with-pgsql'
      args.push "--with-pgsql"
      args.push "--with-pdo-pgsql"
    end
